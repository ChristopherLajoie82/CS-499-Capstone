...
class JobRepository:
    ...
    def get_job_summaries(self) -> List[Dict]:
        ...
        colors is a comma separated list of distinct color names.
        """
        cur = self._conn.cursor()
        cur.execute("""
            SELECT
                ...
                MIN(created_at) AS first_mixed,
                MAX(created_at) AS last_mixed,
                GROUP_CONCAT(DISTINCT TRIM(IFNULL(color_name,''))) AS colors_csv
            FROM jobs
            GROUP BY job_number
        ...
        out = []
        for r in rows:
            colors_csv = (r["colors_csv"] or "").strip(",")
            # de duplicate defensively
            colors = []
            if colors_csv:
                seen = set()
                for c in [c.strip() for c in colors_csv.split(",") if c.strip()]:
    ...
    def get_all_jobs(self) -> List[Dict]:
        """Get all individual job entries with full details"""
        cur = self._conn.cursor()
        cur.execute("""
            SELECT id, job_number, manufacturer, paint_code, paint_version,
                   color_name, mixed_amount, mixed_by, notes, created_at, updated_at
