...
class SettingsManager:
    ...
    def get_operators(self) -> List[str]:
        """Get list of operators (mixed_by)"""
        return self.settings.get("mixed_by", self.defaults["mixed_by"])
    
    def add_operator(self, name: str) -> bool:
        """Add a new operator"""
        if name and name not in self.settings["mixed_by"]:
            self.settings["mixed_by"].append(name)
            self.settings["mixed_by"].sort()
            self.save_settings()
            return True
        return False
